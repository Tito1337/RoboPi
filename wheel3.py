...
def setup():
    global RotaryLastState1, RotaryLastState2, RotaryCount1, RotaryCount2, InstructionCount1, InstructionCount2
...
def moveUntilCount(dir,count1, count2, conti=True):
    global RotaryCount1, RotaryCount2, InstructionCount1, InstructionCount2
    if(conti):
        InstructionCount1 = InstructionCount1 + count1
        InstructionCount2 = InstructionCount2 + count2
    else:
        sleep(0.5)
        InstructionCount1 = RotaryCount1 + count1
        InstructionCount2 = RotaryCount2 + count2
    if(dir=='forward'):
        M1forward()
        M2forward()
    elif(dir=='backwards'):
    ...
    while not (done1 and done2):
      if(RotaryCount1 > InstructionCount1):
        M1stop()
        done1 = True
      if(RotaryCount2 > InstructionCount2):
        M2stop()
        done2 = True
...
if __name__ == '__main__':
    ...
    try:
        ...
        moveUntilCount('left', 15, 14, False)
        moveUntilCount('forward', 10, 10, False)
        for x in range(1, 9):
           moveUntilCount('forward', 10, 10)
        moveUntilCount('right',14,15, False)

        sleep(1)
        while True:
            forwardUntilObstacle()
            moveUntilCount('backwards', 5, 5, False)
            #Contourne par la droite
            moveUntilCount('right',15,14, False)
            moveUntilCount('forward', 10, 10, False)
            for x in range(1, 5):
                moveUntilCount('forward', 10, 10)
            moveUntilCount('left', 14, 15, False)
